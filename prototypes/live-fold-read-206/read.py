...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--out", type=Path, default=Path("prototypes/live-fold-read-206"))
    ...
    (args.out / "206-read.json").write_text(json.dumps(payload, indent=2))
    np.savez_compressed(
        args.out / "206-per-tick.npz",
    ...
    print(f"\n  Written to {args.out}/206-read.json and 206-per-tick.npz")
