...
about **1.05x** the median. That was thinner than any surface in this repo had admitted, and it is
stated here rather than left to be re-derived.

**[#548](https://github.com/NGL321/patchworks/issues/548) relieved it, and the relief is per edge.**
With interior widths allocated rather than set, the narrowest chain-carrying interior lane on
`DEFAULT_SPEC` is **5**, a ceiling of `d_box < 2.5` — **~1.75x** #132's median and clear of its upper
quartile of 1.53 outright. The ~1.05x above is the *history* of this margin, not its current value,
and the interior lane is no longer the thinnest number in the design. **This is not a licence, and
the caveats below are exactly why**: the reading moved because the lane moved, and the analogy it
rests on did not become any better sourced. **Two caveats stop it being a contradiction, and both
...
> Everything below reasons about a *pair* of global constants, and there is no such pair any more:
> interior widths are allocated per edge and `interior_m` is gone. **Nobody re-derives
> `(interior_m, boundary_m)`**, so read this for the mechanism it explains — that a cell's rim edges
> can fill its stalk before an interior edge is counted, which is why lateral and rim *width* is
> where privacy is won or lost — and not as a recipe. The clause that sent a session here to re-derive
> the pair has been struck.
...
**And now for a better reason than the record had, which is about cost rather than about reach.**
The argument that used to stand here — that `m ≲ (n−1)/deg` gives `m/n ≲ 1/deg` independently of `n`,
so raising `n` *"buys absolute width and nothing for the angles"* — **is false, and was falsified
twice**: [#537](https://github.com/NGL321/patchworks/issues/537) measured composed effective rank at
equal ratio `m/k_v = 0.5` and found a factor of **25** between `(m=9, k_v=18)` and `(m=3, k_v=6)`, and
[#539](https://github.com/NGL321/patchworks/issues/539) reached the same place in closed form via
Forrester (2015) on truncated orthogonal matrices. **Absolute `m` governs the composite, not the
ratio.** So `n` does reach.

It is refused anyway, and [#540](https://github.com/NGL321/patchworks/issues/540) §3 says why in one
comparison: **`m = 10` at today's `n = 32` reads composed ER 2.561; `n = 128` at today's degree reads
2.544.** They are the same number, and the second costs **16× every restriction map's parameter
count** and moves a constant this whole spec is written against. **`n` is not a lever, it is a
permission slip** — it moves composed rank only by permitting a wider `m`, and the construction
invariant can permit that directly at no parameter cost at all. A session arriving here wanting `n`
should want the invariant instead ([#556](https://github.com/NGL321/patchworks/issues/556)).
...
**#548 is the same statement carried one step further, and it is worth being precise about what
changed.** #474 made `p_v` *set* rather than residual by deriving the widths from the invariant. #548
removed the widths as a thing to derive at all: the invariant now sizes each lane directly, so `p_v`
is set by the invariant with no constant in between. That is why the whole graph could be rewired
without the zero row returning — and it is also why the *sum* moved. **A quantity determined by a
constraint is guaranteed at its floor and unowned above it**, and the 364 dimensions the reallocation
...
- **Reach and squeeze are the same quantity, and the relays that would relieve it are the ones
  ...
  **These were `12,288 → 2,120 → 280 → 80` and a 154:1 squeeze** at `interior_m = 4`,
