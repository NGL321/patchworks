...
**When you are about to explain something rather than change it**, also read **`docs/analogies/`** — a small collection of analogies that have been built with the user and tested against readings. Reach for one of those rather than inventing a fresh image, so that an explanation given today uses the same one as an explanation given six months ago. `docs/analogies/README.md` says how they may and may not be used; the short version is that an analogy generates questions and never settles one.
