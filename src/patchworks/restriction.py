...
#: @type stipulated
#: @flexibility measured, and now settled in both directions: the overlap count runs 2.42 at the rim to 1.75-1.98 in the core (#182) against a ~1.05 floor set by effective rank, which is what made 2 conservative when it was adopted -- it is what levels 4-7 already satisfy untouched. **#184's precondition is discharged, not open**: the cost to cross-edge alignment it wanted measured before tightening was measured and the pre-registered condition **fired** at 4.06x -- 3.49x/3.50x taught against a 14.20x baseline, below its own 4.04x untrained arm (#538, `prototypes/falsifier-538/`) -- on a surface where ADR-0032's floor moved the sender-side half too, so #538 alone left `c` indicted but not isolated. **The control has since reported, and isolated `c` does nothing. Loosening is worthless**: c = 1, 2, 3, 4, 6, 8, 12 give identical composed ER medians to seven significant figures at construction and at every checkpoint of both 100k arms (#537); composed rank is a function of (m, k_v, hops) alone, and the 150 cells c governs are slack by 4.4x. **Tightening is harmful**: c = 1 in circuit -- projection and gain together, ungated, 100k, both arms -- reads composed ER excess over one ~1,990x (baseline) and ~28,800x (winner) *worse* than c >= 2 (#551), structurally, because c caps *incoherence* and a hop is the cosines of the principal angles between a relay cell's two carried subspaces, so pushing a cell's incident maps apart destroys the material composed rank is made of (leading cosine 0.818/0.794 -> 0.353/0.370). The earlier sweep that put c = 1 ahead was a post-hoc re-projection of a surface trained at c = 2 that never touched the gain, and overstated it by ~3e6x. **The cap saturates in the tail, not the population**: baseline Gram p90 0.25 -> 0.9998 by 100k, cells at the cap 4 -> 21, cells over half 7 -> 153 (all of them), while the median stays pinned at 0.2500 at every checkpoint of both arms -- the typical governed cell is as slack at 100k as at construction. Kept unchanged at 2 (#547, #551); not a lever in either direction, and #228's c_v = deg(v) survives because c does
